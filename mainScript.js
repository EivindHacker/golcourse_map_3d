...
function LoadModels() {
	...
	loader.load(`./holes/hole${hole.holeNr}.gltf`, GLTFLoadDone);
}
...
function AnimateObjects() {
	...
	if (forward || backward) {
		if (
			Math.sqrt((green.position.x - cartPosX) * (green.position.x - cartPosX) + (green.position.z - cartPosZ) * (green.position.z - cartPosZ)) <
			hole.greenRadius
		) {
